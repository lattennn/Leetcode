                ...
                for(num=0;listnum!=0;listnum--){
                    ...
                    for(k=0;k<4;k++){
                        // check four directional adjacent nodes of (r,c)                                                 // if( grid[i+rmove[k]][j+cmove[k]] == '1' ){ WRONG
                            if( r+rmove[k]>=0 && r+rmove[k] < gridSize && c+cmove[k]>=0 && c+cmove[k]< (*gridColSize) && grid[r+rmove[k]][c+cmove[k]] == '1' && visited[r+rmove[k]][c+cmove[k]]==0 ){ // if it's land '1' add it to the queue
